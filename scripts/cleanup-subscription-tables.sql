-- 清理会员相关的表和功能
DROP TABLE IF EXISTS payment_records CASCADE;
DROP TABLE IF EXISTS subscription_history CASCADE;
DROP TABLE IF EXISTS user_subscriptions CASCADE;

-- 删除会员相关的函数
DROP FUNCTION IF EXISTS check_subscription_status(UUID);
DROP FUNCTION IF EXISTS get_user_profile(UUID);
DROP FUNCTION IF EXISTS get_leaderboard(TEXT, INTEGER, UUID);
DROP FUNCTION IF EXISTS expire_subscriptions();

-- 删除会员相关的视图
DROP VIEW IF EXISTS user_profiles CASCADE;
DROP VIEW IF EXISTS active_subscriptions CASCADE;
DROP VIEW IF EXISTS expiring_subscriptions CASCADE;
DROP VIEW IF EXISTS user_statistics CASCADE;

-- 重新创建简化的用户统计函数
CREATE OR REPLACE FUNCTION calculate_streak(user_id UUID)
RETURNS INTEGER AS $$
DECLARE
  streak_count INTEGER := 0;
  current_date DATE := CURRENT_DATE;
  check_date DATE;
BEGIN
  check_date := current_date;
  
  LOOP
    IF EXISTS (
      SELECT 1 FROM daily_checkins 
      WHERE daily_checkins.user_id = calculate_streak.user_id 
      AND date = check_date 
      AND relapsed = false
    ) THEN
      streak_count := streak_count + 1;
      check_date := check_date - INTERVAL '1 day';
    ELSE
      EXIT;
    END IF;
  END LOOP;
  
  RETURN streak_count;
END;
$$ LANGUAGE plpgsql;

-- 更新用户统计数据的函数
CREATE OR REPLACE FUNCTION update_user_stats(user_id UUID)
RETURNS void AS $$
DECLARE
  new_streak INTEGER;
  new_total INTEGER;
  new_level INTEGER;
BEGIN
  new_streak := calculate_streak(user_id);
  
  SELECT COUNT(*) INTO new_total
  FROM daily_checkins 
  WHERE daily_checkins.user_id = update_user_stats.user_id 
  AND relapsed = false;
  
  new_level := CASE 
    WHEN new_total >= 365 THEN 15
    WHEN new_total >= 300 THEN 14
    WHEN new_total >= 270 THEN 13
    WHEN new_total >= 240 THEN 12
    WHEN new_total >= 210 THEN 11
    WHEN new_total >= 180 THEN 10
    WHEN new_total >= 150 THEN 9
    WHEN new_total >= 120 THEN 8
    WHEN new_total >= 90 THEN 7
    WHEN new_total >= 60 THEN 6
    WHEN new_total >= 30 THEN 5
    WHEN new_total >= 21 THEN 4
    WHEN new_total >= 14 THEN 3
    WHEN new_total >= 7 THEN 2
    ELSE 1
  END;
  
  UPDATE users 
  SET 
    current_streak = new_streak,
    total_days = new_total,
    level = new_level,
    updated_at = NOW()
  WHERE id = user_id;
END;
$$ LANGUAGE plpgsql;
