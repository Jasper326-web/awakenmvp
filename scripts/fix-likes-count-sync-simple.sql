-- 修复 community_posts 的 likes_count 与 community_likes 表同步（简化版）
-- 在 Supabase 控制台中运行此脚本

-- 1. 为 community_likes 表添加 updated_at 字段（如果不存在）
ALTER TABLE community_likes 
ADD COLUMN IF NOT EXISTS updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();

-- 2. 首先更新所有帖子的点赞数，基于 community_likes 表的实际数据
